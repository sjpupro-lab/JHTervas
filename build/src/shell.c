/*
 * shell.c — Phase-8/10: CanvasOS Shell
 *
 * Provides:
 *   - Builtin commands (ps, hash, info, ls, cd, mkdir, rm, echo, cat, kill, help, exit)
 ...
 *   - PixelCode fallback for unknown commands
 */
...
static int shell_exec_builtin(Shell *sh, EngineContext *ctx, const char *line) {
    ...
    if (strcmp(cmd, "timewarp") == 0) {
        ...
        if (sscanf(arg, "%u", &tick) == 1)
            return timewarp_goto(&sh->timewarp, ctx, tick);
